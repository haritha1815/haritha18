library(readr)
setwd()
cococola<-read.csv(file.choose())
View(cococola) 
windows()
plot(quatder$sales...000.,type="o")
...
plot(quad.sales$residuals)

acf(quad.sales$residuals, lag.max = 10) # take all residual value of the model built & plot ACF plot

A <- arima(quad.sales$residuals, order = c(1,0,0))
A$residuals
